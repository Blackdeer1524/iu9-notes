## 7.4. Неоднородные граничные условия
$$\begin{cases}
\rho P[U] = LU \\
...
$$L U(x) = 0, x \in (a,b)$$
При этом
$$L U(x) = \frac{d}{dx}(K(x) \frac{dU}{dx}) - q(x)U$$
Получаем
$$\frac{d}{dx}(K(x) \frac{dU}{dx}) - q(x)U(x) = 0 \tag{1}$$
Условия $K(x)$ 
$$\begin{cases}
K(x) > 0, x \in (a,b) \qquad (a)\\
K(x) = (x-a) \varphi(x) \qquad (b)
\end{cases} \tag{2}$$ 
...
**Лемма**
Пусть $U_1(x), U_2(x)$ - 2 ЛНЗ решения уравнения $(1)$ и $K(x)$ удовлетворяет условиям (2). Тогда, если $u_{1}(x)$ - ограниченное решение, имеющее конечный предел в т. $x=a$, то второе решение $u_{2}(x)$ при $x \rightarrow a$ является неограниченным. Причем
* если $u_1(a) \not = 0$, то $u_{2}(x)$ имеет в $a$ логарифмическую особенность
* если $u_1(a)$ имеет нуль $\nu$-ого порядка, то $u_2(x)$ в $a$ имеет полюс $\nu$-го порядка. 


# 9. Цилиндрические функции
## 9.1. Уравнение Бесселя
$$\frac{d}{dx} (K(x) \frac{dU}{dx}) - q(x)U = 0$$
Если в этом уравнении, $K(x) = x, q(x) = -x + \frac{\nu^2}{x}$, то уравнение следующего вида
...
$$\Gamma(z) = \frac{1}{e^{i2\pi z} - 1} \int_L e^{-t} t^{z-1}dt, z \in \mathbb C \setminus \mathbb Z$$
...
![[Pasted image 20250609161340.png]]
