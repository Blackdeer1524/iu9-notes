...
## 10.1 Определение
Система $\{p_n\}$ полиномов всех степеней, заданных на $[a,b]$ называют **системой ортогональных полиномов**, если они ортогональны на этом отрезке с весом $\rho(x)$, удовлетворяющим на интервале $(a,b)$ **диффуру Пирсона**
...
Что нас волнует? 
$$\frac{d^m}{dx^m}[p_n] = \frac{d^m}{dx^m}(a_nx^n...)$$
...
$$ = \frac{a_n}{2^n} 2^{\alpha+n} \int_0^2(1 - \frac t2)^{\alpha+n} t^{\beta + n}dt$$
